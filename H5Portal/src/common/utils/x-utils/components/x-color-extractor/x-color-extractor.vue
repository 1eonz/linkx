<template>
  <view class="x-color-extractor">
    <view class="color-extractor-view" :style="[{ width, height }, customStyle]">
      <canvas
        :style="{ width, height, transform: `translateX(${offsetCorrection})` }"
        @touchend="touchend"
        id="colorCanvas"
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        let ctx = null;
        let width = (this._width = this.width.includes('rpx')
          ? uni.upx2px(parseInt(this.width))
          : parseInt(this.width));
        let height = (this._height = this.width.includes('rpx')
          ? uni.upx2px(parseInt(this.height))
