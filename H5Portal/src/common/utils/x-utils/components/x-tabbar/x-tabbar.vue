  ...
  <view class="custom_tabbar" :style="{ width: '1px', height: `calc(${height} + ${safeBottom})` }">
    ...
    >
      <view class="item" v-for="i in list" @click="$emit('tab-click', i)">
        <view
          v-if="i.iconPath && i.selectedIconPath"
          class="image"
          :style="[{ marginBottom: spacing }]"
        >
          <image
            :style="[{ width: iconWidth, height: iconWidth }]"
            :src="i.pagePath == activePath ? i.selectedIconPath : i.iconPath"
            mode="widthFix"
          ></image>
          <view
            v-if="i.badge && i.badge > 0"
            class="badge"
            :style="[{ color: badgeFontColor, background: badgeBgColor, badgeStyle }]"
          >
        ...
        <text
          :style="[
            {
              color: i.pagePath == activePath ? selectedColor : color,
              fontSize,
            },
          ]"
          >{{ i.text }}</text
