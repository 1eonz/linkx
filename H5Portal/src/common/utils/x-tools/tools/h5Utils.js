...
export const debug = async (tip = true) => {
  const erudaList = ['https://unpkg.com/eruda', 'https://cdn.jsdelivr.net/npm/eruda', '/eruda.js'];
  let idx = 0;
  while (erudaList[idx]) {
    try {
...
export const wxConfig = (params) => {
  ...
  jWeixin.config({
    ...
    jsApiList: ['updateTimelineShareData', 'updateAppMessageShareData', 'chooseWXPay'],
  });
};
