...
export const toast = (title, options = { duration: 2000, icon: 'none' }) =>
  uni.showToast({ title, fail: console.log, ...options });

export const makePhoneCall = (phoneNumber) => uni.makePhoneCall({ phoneNumber, fail: console.log });

export const previewImage = (urls, current, options) =>
  uni.previewImage({ urls, current, fail: console.log, ...options });

export const copy = (str, options) =>
  uni.setClipboardData({ data: String(str), fail: console.log, ...options });

export const openMap = (lng, lat, name, address, options) =>
  uni.openLocation({
...
// 有无空字段
export const hasEmptyField = (obj, excludedKeys = []) =>
  getEmptyValuePropertyPath(obj, excludedKeys);
