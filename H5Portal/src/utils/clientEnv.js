...
// 模块加载时计算一次并缓存，会话内浏览器/屏幕不变
const _cachedBrowserInfo = _parseBrowserInfo();
const _cachedScreenInfo = _parseScreenInfo();

// 操作系统信息缓存，因需异步调用SDK获取deviceType，单独管理
let _cachedOSInfo = _parseOSInfoFromUA();
...
// 异步预热操作系统信息，优先从WeSpaceSDK获取deviceType
export function preheatOSInfo() {
  if (typeof window === 'undefined' || !window.WeSpaceSDK?.getVersion) return;
  withTimeout(
    window.WeSpaceSDK.getVersion(),
    500,
    null,
  ).then((versionInfo) => {
    if (versionInfo?.deviceType) {
      _cachedOSInfo = versionInfo.deviceType;
    }
    if (versionInfo?.versionCode) {
      versionCode = versionInfo.versionCode;
    }
  }).catch((e) => {
    console.warn('[clientEnv] SDK getVersion 失败，使用UA解析兜底:', e);
  });
}
