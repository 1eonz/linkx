...
| 参数名           | 类型    | 必填 | 说明                                               |
| ---------------- | ------- | ---- | -------------------------------------------------- |
| type             | number  | 否   | 群组类型：1-群聊组，2-警单建群，3-协同组，默认值 1 |
| groupName        | string  | 否   | 群组名称                                           |
| ticketId         | number  | 否   | 警单ID（type=2 时必填）                            |
| ownerId          | string  | 否   | 创建人ID（type=2 时必填）                          |
| departmentCode   | string  | 否   | 部门编码（type=2 时必填）                          |
| location         | string  | 否   | 位置：经纬度，格式：经度,纬度                      |
| introduction     | string  | 否   | 介绍                                               |
| needSelectMember | boolean | 否   | 是否弹出界面选人                                   |
...
// 创建普通群组
const result2 = await sdk.createGroup({
  type: 1,
  groupName: '我的群组',
...
支持两种渠道：当前用户发送（channel=0）和系统发送（channel=1）。支持三种消息类型：文本消息（msgType=1）、彩信消息（msgType=2）、卡片消息（msgType=5）。

**参数**:

| 参数名         | 类型   | 必填 | 说明                                                                  |
| -------------- | ------ | ---- | --------------------------------------------------------------------- |
| channel        | number | 否   | 发送渠道：0-当前用户发送（默认），1-系统发送                          |
...
| msgType        | number | 是   | 消息类型：1-文本消息，2-彩信消息，5-卡片消息                          |
| content        | string | 条件 | 文本内容（msgType=1 文本消息时必填）                                  |
| fileId         | string | 条件 | 彩信文件ID（msgType=2 彩信消息时必填，来自openapi文件上传接口 [POST] /openapi/v1/file/upload 返回的md5）|
| card           | Object | 条件 | 卡片信息（msgType=5 卡片消息时必填，同openCompOfSendCardMsg的传参）  |

> **说明**：`userIdTargets`、`idCardTargets`、`groupIds` 三选一必填。`content`、`fileId`、`card` 按 `msgType` 对应必填。

> **限制**：`msgType=2` 彩信消息仅支持 `channel=1` 系统发送，`channel=0` 当前用户发送暂不支持。

**card 对象属性**:

| 参数名       | 类型   | 必填 | 说明                                                         |
| ------------ | ------ | ---- | ------------------------------------------------------------ |
| level        | string | 否   | 级别：blue/orange/yellow/red（一般/关键/重要/紧急）          |
| title        | string | 否   | 标题                                                         |
...
// channel=1：系统发送卡片消息（通过群组ID）
const result3 = await sdk.sendMsg({
  channel: 1,
  groupIds: ['100001', '100002'],
  msgType: 5,
  card: {
...
```javascript
const success = await sdk.pickFromAlbum();
```
...
录像（通过 file input 调用设备摄像头录像，自动适配鸿蒙/非鸿蒙环境）。

**返回值**: `Promise<File|null>` - 返回录像文件对象，失败返回 null

**示例**:

```javascript
const file = await sdk.recordVideo();
...
- 新增 `sendMsg` 发送IM消息方法（支持文本/彩信/卡片三种消息类型）
