        ...
        <view class="message-left">
          ...
          <view class="message-content">
            <view class="message-header">
              <text class="sender-name">{{ getUserName(message) }}</text>
...
<script setup>
  ...
  const lookFile = (fileContent) => {
    ...
    // 直接使用URL打开，避免blob问题
    openFileDirectly(fileUrl, fileName);
  };

  // 直接使用URL打开文件
  const openFileDirectly = (fileUrl, fileName) => {
  ...
  const onActionSelect = (action, index) => {
    ...
    switch (index) {
      ...
      case 1:
        // 下载并打开
        downloadAndOpen(myFileUrl.value, myFileName.value);
        break;
      case 2:
        // 复制链接
  ...
  // 下载并打开文件（修复blob问题）
  const downloadAndOpen = (fileUrl, fileName) => {
    console.log('下载并打开文件:', fileUrl);

    showLoading.value = true;

    // 使用XMLHttpRequest下载，避免blob URL问题
  ...
  const getUserAvatar = (userId) => {
    ...
    console.info('[群接龙调试-嵌套] getUserAvatar - userId:', userId, '找到member:', member ? JSON.parse(JSON.stringify(member)) : null);
    if (member) {
      // 优先使用协同岗头像，其次使用普通头像
      const avatar = member.collaborationAvatar || member.tumbAvatar || member.avatar || '';
      console.info('[群接龙调试-嵌套] getUserAvatar - 最终avatar路径:', avatar);
      if (avatar) {
    ...
    if (remoteUser?.avatar) {
      return transformImageUrl(`/admin-api${remoteUser.avatar}`);
    }
    return '';
  };
  ...
  const getUserName = (message) => {
    ...
    if (member) {
      ...
      if (message.fromRealUserId) {
        ...
        const realMember = props.memberList.find((m) => m.id == message.fromRealUserId);
      ...
      const result = member?.name || `用户${userId}`;
      return result;
    }
  ...
  const getGroupJoiningContent = (message) => {
    console.info('[群接龙调试-嵌套] getGroupJoiningContent - message:', JSON.parse(JSON.stringify(message)));
    console.info('[群接龙调试-嵌套] getGroupJoiningContent - props.memberList:', JSON.parse(JSON.stringify(props.memberList)));
    const content = parseGroupJoiningMessage(message);
    console.info('[群接龙调试-嵌套] getGroupJoiningContent - 解析后content:', content);
    ...
    if (content.userTxts && Array.isArray(content.userTxts)) {
      ...
      participants = sortedUserTxts.map((userTxt, index) => {
        ...
        console.info('[群接龙调试-嵌套] 参与者 - userTxt:', JSON.parse(JSON.stringify(userTxt)), '匹配member:', member ? JSON.parse(JSON.stringify(member)) : null);
        if (member) {
          ...
          console.info('[群接龙调试-嵌套] 参与者结果 - name:', participantName, 'avatar:', avatar, 'isCollaborationIdentity:', isCollaborationIdentity, 'postName:', postName, 'realName:', realName, 'txt:', txt);
...
<style lang="scss" scoped>
  .chat-history-container {
    ...
    height: 100vh;
    padding-bottom: 250px; /* 250rpx ≈ 125px，但根据底部输入框高度调整为 250px */
    background: #f5f5f5;
  }

  .forward-messages {
    padding: 10px; /* 20rpx = 10px (按 2倍设计稿) */
  }
  ...
  .message-content {
    flex: 1;
    /* max-width: calc(100% - 100rpx); */ /* 100rpx = 50px，可根据需要调整 */
  }
  ...
  .message-bubble {
    ...
    padding: 8px 10px; /* 16rpx=8px, 20rpx=10px */
    position: relative;
