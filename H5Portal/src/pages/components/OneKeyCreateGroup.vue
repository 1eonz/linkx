<template>
  <view class="button-box" v-if="enabled">
    <view class="create-group one-key-create" @click="handleCreateGroup">
      <img
...
<script setup>
  ...
  const props = defineProps({
    ...
    config: {
      type: Object,
      default: () => ({ title: '一键建群', enabled: false }),
    },
    // 字体大小
    fontSize: {
      type: String,
  ...
  // 计算属性：标题
  const title = computed(() => props.config?.title || '一键建群');
  ...
  const _handleCreateGroup = async () => {
    try {
      // 跳转一键建群页面
      const url = pageUrlStore.getFullPageUrl('/pages/createGroup?token=' + props.accessToken);
