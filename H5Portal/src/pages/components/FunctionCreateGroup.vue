...
<script setup>
  ...
  const props = defineProps({
    ...
    config: {
      type: Object,
      default: () => ({ title: '职能建群', enabled: false }),
    },
    // 字体大小
    fontSize: {
      type: String,
  ...
  // 计算属性：标题
  const title = computed(() => props.config?.title || '职能建群');
  ...
  const _handleCreateGroup = async () => {
    try {
      // 跳转职能建群页面
      const url = pageUrlStore.getFullPageUrl('/pages/functionGroup');
      await communicationStore.openUrl(url, null, 'noTitleStyle');

      emit('create-success');
    } catch (error) {
      console.error('职能建群失败:', error);
    }
  };
