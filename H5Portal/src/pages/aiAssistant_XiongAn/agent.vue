...
import { createSessionId } from "@/utils/aiAssistantUtils.js";
...
async function getAgents(init) {
  ...
  try {
    ...
    console.log(`${logPrefix} 调用成功 ✅ 耗时: ${duration}ms, 返回数据: ${allAgents.length}条`);
  } catch (error) {
    // 计算耗时（失败情况）
    const duration = Date.now() - startTime;
    console.error(`${logPrefix} 调用失败 ❌ 耗时: ${duration}ms`, error);
  } finally {
    loading.value = false;
  }
}
...
function openApplyForm(item, event = null) {
  // 阻止事件冒泡，避免触发外层列表点击
  event?.stopPropagation();
  navigateToUrl("/pages/aiAssistantXiongAn/applyPermissionForm?formType=1&params=" + JSON.stringify(item), null, "noTitleStyle");
}
//获取勾选权限
