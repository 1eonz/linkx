...
async function enterChatFromRouteQuery() {
  linkxLog('aiAssistant/index.vue', 'enterChatFromRouteQuery', "开始调用");
  if (!sessionMode.value) return;
  const q = route.query || {};
  ...
  linkxLog('aiAssistant/index.vue', 'enterChatFromRouteQuery', "调用结束");
}

// 从聊天态退回默认态时，用于恢复“历史列表样式”并清理当前会话状态
async function resetToSessionHistoryState() {
  linkxLog('aiAssistant/index.vue', 'resetToSessionHistoryState', "开始调用");
  if (!sessionMode.value) return;
  resetStreamState();
  messages.value = [];
  ...
  linkxLog('aiAssistant/index.vue', 'resetToSessionHistoryState', "调用结束");
}
...
onMounted(async () => {
  ...
  const shouldUseSessionMode = interactionMode === 'session';
  
  // 如果路由参数没有传 sessionMode，但全局配置是 session 模式，则使用全局配置
  if (!route.query.sessionMode && shouldUseSessionMode) {
  ...
  if (sessionMode.value) {
    ...
    if (userInfo.value?.userid) {
      try {
        ...
        console.log("已清理综合模式缓存");
      } catch (error) {
        console.error("清理综合模式缓存失败:", error);
      }
    }
    ...
    if (userInfo.value?.userid) {
      try {
        ...
        console.log("已清理会话模式缓存");
      } catch (error) {
        console.error("清理会话模式缓存失败:", error);
      }
    }
    
    functions.value = await aiStore.getCurrentFunc(agents.value);
    // 综合模式下如果常用智能体列表为空，则显示所有智能体
    if (functions.value.length === 0) {
      functions.value = agents.value.slice(0, 9);
    }
  ...
  // 会话模式：从历史记录页面返回时选中的会话
  registerStorageListener(
      `selectedSession_${userInfo.value?.userid}`,
      async (val) => {
        if (!sessionMode.value) return;
        try {
          const parsed = val ? JSON.parse(decodeURIComponent(escape(val))) : null;
          if (parsed == null || !parsed?.sessionId) return;
          
          console.log("从历史记录页面返回，恢复会话", parsed);
          ...
          currentAgentIndex.value = parsed.agentIndex ?? getDeepSeekAgentIndex(agents.value);
          showSessionFunctions.value = false;

          // 进入聊天态：压栈一条 chat=1 路由
          await pushSessionChatRoute({
            sessionId: currentSessionId.value,
            agentIndex: currentAgentIndex.value,
          });
          ...
          if (userInfo.value?.userid) {
            try {
              ...
              console.log("已清除 selectedSession 存储");
            } catch (error) {
              console.error("清除 selectedSession 存储失败:", error);
            }
          }
          ...
          console.log("解析selectedSession失败", e);
        }
      }
  );
...
async function loadSessionHistories() {
  linkxLog('aiAssistant/index.vue', 'loadSessionHistories', "开始调用");
  ...
  linkxLog('aiAssistant/index.vue', 'loadSessionHistories', "调用结束");
}

// 会话模式：删除某个会话（顶部列表长按触发，按会话ID删除）
async function deleteSession(func) {
  linkxLog('aiAssistant/index.vue', 'deleteSession', "开始调用");
  if (!sessionMode.value) return;
  ...
  try {
    ...
    linkxLog('aiAssistant/index.vue', 'deleteSession', "调用结束");
  } catch (e) {
...
async function handleFunction(func) {
  linkxLog('aiAssistant/index.vue', 'handleFunction', "开始调用");
  if (sessionMode.value) {
  ...
  console.log("综合模式切换智能体后:", {
    funcIndex: func?.index,
    funcIndexType: typeof func?.index,
    currentAgentIndex: currentAgentIndex.value,
    currentAgentIndexType: typeof currentAgentIndex.value,
    agentsListSample: agents.value.slice(0, 2).map(a => ({ index: a.index, name: a.name, indexType: typeof a.index }))
  });
  aiStore.setCurrentAgent(currentAgentIndex.value);
  linkxLog('aiAssistant/index.vue', 'handleFunction', "调用结束");
}
...
async function getUserInfo() {
  linkxLog('aiAssistant/index.vue', 'getUserInfo', "开始调用");
  try {
    ...
    linkxLog('aiAssistant/index.vue', 'getUserInfo', "调用结束");
  } catch (error) {
...
async function callAiApi(message) {
  linkxLog('aiAssistant/index.vue', 'callAiApi', "开始调用");
  try {
    ...
    console.log("callAiApi 调用时智能体信息:", {
    ...
    if (userInfo.value?.userDepartments?.length) {
      ...
      linkxLog('aiAssistant/index.vue', 'callAiApi', "newTask调用开始");
      const newTaskData = await aiApi.newTask(params);
      linkxLog('aiAssistant/index.vue', 'callAiApi', "newTask调用结束");
    ...
    // 使用流式输出，传递当前智能体 index
    await startStreamOutput(currentTaskId.value, false, fallbackAgentIndex);
    isReceiving.value = false;
    scrollToBottom();
  } catch (error) {
...
// 开始流式输出
async function startStreamOutput(taskId, isHistory = false, agentIndex = null) {
  // bugfix: 暂停，输入新问题，每次问答后，while循环里面第2/3次调用的getUnswer 传入taskId是上一次问答的
  // 在循环开始前，currentLoopTaskId 捕获当前的 taskId 到局部变量，用于后续比对
  linkxLog('aiAssistant/index.vue', 'startStreamOutput', "开始调用");
  ...
  if (!isHistory) {
    ...
    console.log("startStreamOutput 创建 AI 消息:", {
      agentIndex,
      agentIndexType: typeof agentIndex,
      tempMessage
    });
  ...
  while (true) {
    try {
      ...
      linkxLog('aiAssistant/index.vue', 'startStreamOutput', "getAnswer调用开始");
      const res = await aiApi.getAnswer({ id: unref(taskId) });
      linkxLog('aiAssistant/index.vue', 'startStreamOutput', "getAnswer调用结束");
...
async function startStreamFromSavedState(taskId) {
  ...
  // 这样可以避免重复回答的问题
  // 确保已显示内容正确更新到消息中
  linkxLog('aiAssistant/index.vue', 'startStreamFromSavedState', "开始调用");
  if (streamState.value.displayedContent) {
    const lastIndex = messages.value.length - 1;
    if (lastIndex >= 0 && messages.value[lastIndex]?.type === "ai") {
      messages.value[lastIndex].content = marked.parse(streamState.value.displayedContent);
  ...
  linkxLog('aiAssistant/index.vue', 'startStreamFromSavedState', "调用结束");
}

// 继续未完成的流式输出（基于保存的状态继续）
async function continueStreamOutput(taskId) {
  linkxLog('aiAssistant/index.vue', 'continueStreamOutput', "开始调用");
  // bugfix: 暂停，输入新问题，每次问答后，while循环里面第2/3次调用的getUnswer 传入taskId是上一次问答的
  // 在循环开始前，currentLoopTaskId 捕获当前的 taskId 到局部变量，用于后续比对
  const currentLoopTaskId = taskId;
  const INTERVAL = 1000;
  ...
  linkxLog('aiAssistant/index.vue', 'continueStreamOutput', "调用结束");
}
...
function updateMessageContent() {
  linkxLog('aiAssistant/index.vue', 'updateMessageContent', "开始调用");
  const lastIndex = messages.value.length - 1;
  ...
  if (shouldUpdate && messages.value[lastIndex]?.type === "ai") {
    ...
    saveConversationState();
    linkxLog('aiAssistant/index.vue', 'updateMessageContent', "调用结束");
  }
}

// 暂停流式输出
function pauseStream() {
  linkxLog('aiAssistant/index.vue', 'pauseStream', "开始调用");
  if ((streamState.value.isStreaming || streamState.value.pendingContent || streamState.value.timer) && !streamState.value.isPaused) {
    ...
    linkxLog('aiAssistant/index.vue', 'pauseStream', "调用结束");
  }
}

// 继续流式输出
function resumeStream() {
  linkxLog('aiAssistant/index.vue', 'resumeStream', "开始调用");
  if (streamState.value.isPaused) {
    streamState.value.isPaused = false;
  ...
  linkxLog('aiAssistant/index.vue', 'resumeStream', "调用结束");
}
...
async function saveSummary(modeName, sessionId, summary) {
  ...
  linkxLog('aiAssistant/index.vue', 'saveSummary', "调用结束");
}

// 构建历史会话列表：将摘要Map转为数组并补齐展示信息
function buildSessionHistoryList(summaryMap = {}) {
  linkxLog('aiAssistant/index.vue', 'buildSessionHistoryList', "开始调用");
  const deepseekIndex = getDeepSeekAgentIndex(agents.value);
  ...
  linkxLog('aiAssistant/index.vue', 'buildSessionHistoryList', "调用结束");
...
async function restoreConversation() {
  linkxLog('aiAssistant/index.vue', 'restoreConversation', "开始调用");
  try {
    ...
    linkxLog('aiAssistant/index.vue', 'restoreConversation', "调用成功");
  } catch (error) {
    console.error("恢复对话状态失败:", error);
    linkxLog('aiAssistant/index.vue', 'restoreConversation', "调用失败");
  }
...
const saveConversationState = async () => {
  linkxLog('aiAssistant/index.vue', 'saveConversationState', "开始调用");
  ...
  linkxLog('aiAssistant/index.vue', 'saveConversationState', "调用结束");
};

// 从会话模式的子组件返回：退出当前智能体视图，回到初始列表样式
// 注意：现在 ChatHistorys.vue 是独立页面，返回逻辑已迁移到各页面组件中
// 此函数保留仅用于兼容性，实际不再使用
async function handleBackFromChild() {
  console.log("handleBackFromChild 已废弃，请使用页面组件的 handleBack");
}

function copyText(msg){
  linkxLog('aiAssistant/index.vue', 'copyText', "开始调用");
  ...
  linkxLog('aiAssistant/index.vue', 'copyText', "调用结束");
}
