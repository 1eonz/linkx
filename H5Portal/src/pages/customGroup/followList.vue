            ...
            <view class="group-item-content">
              <view class="person-avatar">
                ...
                <img
                  ...
                  style="border-radius: 18%"
                />
                <img v-if="item?.state" class="avatar-state" src="@/assets/svg/admin-state.svg" alt=""/>
...
<script setup>
  ...
  import { usersPageOfType } from '@/common/api/customGroup.js';
  ...
  const searchPersons = () => {
    ...
    personList.value = personList.value.filter((item) => item.name.includes(keywords.value));
  };
  const handleCancel = () => {
    listParams.value.pageNo = 1;
    getUserList();
  ...
  async function getUserList(isLoadMore = false) {
    ...
    try {
      let res = await usersPageOfType(userInfo?.userid, {
        ...listParams.value,
      });
      ...
      if (res) {
        ...
        const records = data.map((item) => {
          return {
            ...item,
            id: item.friendId,
            departmentFullName: getFullPathName(item.department),
  ...
  // 部门全路径
  const getFullPathName = (item) => {
    if (item.length) {
      return item[0].fullPathName.split(',').join('/');
    } else {
      return '';
    }
  };
