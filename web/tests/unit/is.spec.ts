...
describe('is —— 基于 Object.prototype.toString 的类型判断', () => {
  ...
  it('传入空字符串作为 type 时返回 false（除非 val 是空字符串的 toString）', () => {
...
describe('isPromise', () => {
  // 注意: 在 happy-dom 环境下，Object.prototype.toString.call(Promise) 返回 [object Object]
  // 而非真实浏览器中的 [object Promise]，因此 isPromise 在 happy-dom 中返回 false
  // 这是测试环境与真实浏览器的差异，测试应反映实际运行时行为
  it('标准 Promise 在 happy-dom 环境下返回 false（环境差异）', () => {
    expect(isPromise(Promise.resolve(1))).toBe(false);
  });

  it('new Promise 在 happy-dom 环境下返回 false（环境差异）', () => {
  ...
  it('thenable 对象（含 then 和 catch 函数）返回 false', () => {
    ...
    // isPromise 内部要求 is(val, 'Promise') 为 true
    // 普通 thenable 对象 toString 不是 [object Promise]，所以为 false
  ...
  it('Promise.resolve 的结果在 happy-dom 环境下返回 false（环境差异）', async () => {
...
describe('isWindow', () => {
  // 注意: 在 happy-dom 环境下，Object.prototype.toString.call(window) 返回 [object global]
  // 而非真实浏览器中的 [object Window]，因此 isWindow 在 happy-dom 中返回 false
  it('happy-dom 环境下 isWindow(window) 返回 false（环境差异）', () => {
...
describe('isElement', () => {
  // 注意: isElement 内部调用 isObject(val)，而 isObject 用 Object.prototype.toString 判断
  // 在 happy-dom 中，DOM 元素的 toString 返回 [object HTMLDivElement] 而非 [object Object]
  // 因此 isElement 在 happy-dom 中对真实 DOM 元素返回 false
  it('真实 DOM 元素在 happy-dom 环境下返回 false（环境差异）', () => {
    const div = document.createElement('div');
    expect(isElement(div)).toBe(false);
  });

  it('document.body 在 happy-dom 环境下返回 false（环境差异）', () => {
    expect(isElement(document.body)).toBe(false);
  });

  it('自定义含 tagName 属性的普通对象返回 true', () => {
    const fake = { tagName: 'DIV' };
    // 普通对象的 toString 是 [object Object]，isObject 返回 true
...
describe('isOnline', () => {
  describe('bizStatus 在线状态', () => {
    ...
    ])('isOnline(%s) 应返回 true', (_name, data) => {
      expect(isOnline(data)).toBe(true);
    });
