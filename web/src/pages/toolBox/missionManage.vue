<script lang="ts" setup>
  ...
  import { useMissionStore, useMonitorStore } from '@/store';

  const { t } = useI18n();
  const monitorStore = useMonitorStore();
  const missionStore = useMissionStore();
  const isShow = computed(() => {
    return monitorStore.allBtnShowType === 'mission';
  });
  const count = computed(() => missionStore.missionTotal.pending);
  ...
  function handleClick() {
    monitorStore.setAllBtnShowType('mission');
  }
</script>

<template>
  <div class="mission-manage-tools">
    <TdTooltip :content="t('homePage.menus.missionList')" placement="left">
      <div class="tool-btn" @click="handleClick">
        <ElBadge :hidden="count === 0" :max="99" :value="count">
          <Icon
            class="icon-tool"
            :name="isShow ? 'map_feature1_active' : 'map_feature1'"
            prefix="bigScreen"
          />
        </ElBadge>
      </div>
    ...
    <TdFrameBox
      ...
      :title="t('homePage.menus.missionList')"
      @close-frame-box="closeFrame"
    >
      <Mission :show-tab="false" />
    </TdFrameBox>
  </div>
</template>

<style lang="less" scoped>
  .mission-manage-tools {
    position: relative;
    display: flex;
    align-items: center;
