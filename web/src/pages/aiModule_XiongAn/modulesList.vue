...
<script lang="ts" setup>
  ...
  function initDefaultData() {
    ...
    // 但仍需要更新名称
    if (props.activeModuleId && props.selectedModules.some((m) => m.index === props.activeModuleId)) {
  ...
  function useModule(item) {
    ...
    if (props.showAsContent) {
      ...
      const index = currentSelected.findIndex((child) => child.index === item.index);
      if (index > -1) {
  ...
  function getText(item) {
    ...
    const index = props.selectedModules.findIndex((child) => child.index === item.index);
