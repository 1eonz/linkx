const modules = import.meta.glob('@/assets/images/emoji/*.png', { eager: true });
const bigModules = import.meta.glob('@/assets/images/bigEmoji/*.png', { eager: true });
...
export const loadEmoji = async () => {
  for (const i of emojiList) {
    const r = await import(`@/assets/images/emoji/${i}.png`);
