<script setup lang="ts">
  ...
  function handleClick(e) {
    if (props.trigger === 'click' && !props.disable) {
      triggerMenu(e);
    }
  }
...
<template>
  ...
  <teleport v-if="showMenu" to="body">
    <div
      ref="menuRef"
      v-clickOutside="() => (showMenu = false)"
      class="dropdown-menu-content"
      :class="{ 'menu-content-dark': !isLight }"
    >
      <div v-for="(item, index) in options" :key="index" class="item">
        <div class="item-wrapper" @click.stop="(e) => itemClick(e, item)">
          ...
          ></div>
          <span :class="{ 'span-label': isLight }">{{ item.label }}</span>
          <Icon
            v-if="item.children"
            class="expand"
...
<style scoped lang="less">
  ...
  .dropdown-menu-content {
    ...
    // border: 1px solid rgb(38 78 209);
    // border-radius: 2px;
    ...
    .item {
      ...
      .item-wrapper {
        ...
        &:hover {
          background: linear-gradient(90deg, rgb(29 172 238 / 80%) 0%, rgb(42 165 236 / 28%) 100%);
