<script lang="ts" setup>
  ...
  import { useImgZoomDrag } from '@/hooks';
  ...
  const emit = defineEmits(['closeDialog', 'maximize']);
  ...
  function getImageScaleToStage(stageWidth, stageHeight) {
    const scale = Math.min(stageWidth / imgObj.width, stageHeight / imgObj.height);
    return scale;
  }
  ...
  function itemChange(index) {
    ...
    preloadImg(url, () => {
      ...
      nextTick(() => {
        imgObj = new Image();
        imgObj.src = url;
        imgObj.width = imgObj.width || 400;
        imgObj.height = imgObj.height || 300;
        originImg = {
        ...
        useImgZoomDrag(imgDom, dragChange);
        setImageSize();
      });
    });
  }
...
<template>
  <div class="evidence-player" :class="{ 'langer-player': isFull }" @keyup.esc="keyEsc">
    <div class="heard dragger">
      ...
      <div class="right-btn">
        <Icon :name="isFull ? 'minimize' : 'maximize'" @click="handleMaximize" />
        <Icon name="close" @click="closeWindow" />
      </div>
      <svg class="bg-svg">
    ...
    <div class="img-center">
      <ElCarousel
        :arrow="previewSrcList.length === 1 ? 'never' : 'hover'"
        :autoplay="false"
        height="100%"
        :indicator-position="previewSrcList.length === 1 ? 'none' : ''"
        :initial-index="imgIndex"
        trigger="click"
        @change="itemChange"
      >
        <ElCarouselItem v-for="(item, index) in previewSrcList" :key="item">
          <div class="img-box">
            <div v-show="item && imgLoading" :id="`stage${index}`" class="img-item">
              <img
                :id="`img${index}`"
                alt=""
                :src="item"
            ...
            <img v-if="!item" alt="" src="@/assets/images/common/no-image.png" />
          </div>
        </ElCarouselItem>
      </ElCarousel>
    </div>
