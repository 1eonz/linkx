...
public class WebSocketSessionManagerImpl implements WebSocketSessionManager {
    ...
    /**
     * idCard 与 WebSocketSession 映射
     *
     * key：Session 编号
     */
    private final ConcurrentMap<String, CopyOnWriteArrayList<WebSocketSession>> idCardSessions = new ConcurrentHashMap<>();
    ...
    public void addSession(WebSocketSession session) {
        ...
        // 添加到 idCardSessions 中
        if (StringUtils.isNotBlank(user.getIdCard())) {
            idCardSessions.computeIfAbsent(user.getIdCard(), k -> new CopyOnWriteArrayList<>()).add(session);
        }
    ...
    public void removeSession(WebSocketSession session) {
        ...
        // 移除从 idCardSessions 中
        if (StringUtils.isNotBlank(user.getIdCard())) {
            idCardSessions.get(user.getIdCard()).removeIf(session0 -> session0.getId().equals(session.getId()));
        }
        ConcurrentMap<Long, CopyOnWriteArrayList<WebSocketSession>> userSessionsMap = userSessions.get(user.getUserType());
        if (userSessionsMap == null) {
            return;
    ...
    @Override
    public Collection<WebSocketSession> getSessionList(String idCard) {
        CopyOnWriteArrayList<WebSocketSession> sessions = idCardSessions.get(idCard);
        return CollUtil.isNotEmpty(sessions) ? new ArrayList<>(sessions) : new ArrayList<>();
    }
