package com.chinasoft.cloud.framework.websocket.core.sender;
...
public abstract class AbstractWebSocketMessageSender implements WebSocketMessageSender {
    ...
    @Override
    public void sendByIdCard(String idCard, String messageType, String messageContent) {
        Collection<WebSocketSession> sessionList = sessionManager.getSessionList(idCard);
        if (CollUtil.isEmpty(sessionList)) {
            if (log.isDebugEnabled()) {
                log.debug("[sendByIdCard][idCard({}) messageType({}) messageContent({}) 未匹配到会话]",
                        idCard, messageType, messageContent);
            }
            return;
        }
        // 2. 执行发送
        doSend(sessionList, messageType, messageContent);
    }
